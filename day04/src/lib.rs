...
impl Passport {
    fn from_batch(batch_record: Vec<&str>) -> Result<Passport, &str> {
        ...
        // Concatenate all lines in batch record
        let mut cat = String::new();
        for line in batch_record {
            cat.push_str(line);
            cat.push(' ');
        }
        // split batch record into parts and extract key:value separated values from parts
        let parts: Vec<&str> = cat.split_ascii_whitespace().collect();
        for part in &parts {
            let (key, value) = (
                part.split(':').nth(0).unwrap(),
                part.split(':').nth(1).unwrap(),
            );
            match key {
                "byr" => byr = value.parse::<i16>().unwrap(),
                "iyr" => iyr = value.parse::<i16>().unwrap(),
                "eyr" => eyr = value.parse::<i16>().unwrap(),
                "hgt" => hgt = String::from(value),
                "hcl" => hcl = String::from(value),
                "ecl" => ecl = String::from(value),
                "pid" => pid = String::from(value),
                "cid" => cid = Some(String::from(value)),
                _ => println!("Unsupported passport record key {}", key),
            };
        }
        // validate and return appropriately
        if !(parts.len() == 7usize || parts.len() == 8usize) {
            return Err("Missing required field in input");
        } else if hgt == "" || hcl == "" || ecl == "" || pid == "" {
            return Err("Missing required string field");
        } else if byr == -1 || iyr == -1 || eyr == -1 {
            return Err("Missing required year field");
        } else {
            return Ok(Passport {
                byr: byr,
                iyr: iyr,
    ...
    fn valid(&self) -> bool {
        ...
        if self.eyr < 2020 || self.eyr > 2030 {
            validation_errors.push(format!("invalid eyr: {}", self.eyr));
            valid = false;
        }
        ...
        if self.hgt.ends_with("cm") {
            ...
            if height < &150 || height > &193 {
                validation_errors.push(format!("invalid hgt (cm): {}", self.hgt));
                valid = false;
            }
        } else if self.hgt.ends_with("in") {
            let height = &self.hgt.replace("in", "").parse::<i16>().unwrap_or(-1);
            if height < &59 || height > &76 {
                validation_errors.push(format!("invalid hgt (in): {}", self.hgt));
                valid = false;
            }
        } else {
        ...
        if self.hcl.len() == 7 {
            ...
            if color == -1 {
                validation_errors.push(format!("invalid hcl (value): {}", self.hcl));
                valid = false;
            }
        } else {
        ...
        if !valid_colors.contains(&self.ecl) {
            validation_errors.push(format!("invalid ecl: {}", self.ecl));
            valid = false;
        }
...
fn part1(batch: Vec<&str>) -> i32 {
    ...
    // deal with last possible record
    if buf.len() > 0 {
        match Passport::from_batch(buf) {
            Ok(_) => count = count + 1,
            Err(_) => (),
...
fn part2(batch: Vec<&str>) -> i32 {
    // if this pattern repeats again in future days
    // extact out a passport loader function (DRY)
    let mut buf: Vec<&str> = vec![];
    let mut count = 0;
    for line in batch {
        if line == "" {
...
mod tests {
    ...
    #[test]
    fn day04_passport_from_batch_err_string() {
        let input = vec![
            "iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884",
            "hcl:#cfa07d byr:1929",
        ];
        assert_eq!(
            ...
            Err("Missing required string field")
        );
    }

    #[test]
    fn day04_passport_from_batch_err_missing_field() {
        let input = vec![
            "hcl:#cfa07d eyr:2025 pid:166559648",
            "iyr:2011 ecl:brn hgt:59in",
        ];
        assert_eq!(
            Passport::from_batch(input),
            Err("Missing required field in input")
        );
    }
}
